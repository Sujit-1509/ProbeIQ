...
export default function DashboardPage() {
  ...
  return <main className="min-h-dvh px-4 py-6 sm:px-6 sm:py-10"><div className="max-w-6xl mx-auto">
    <div className="pi-surface pi-panel-enter rounded-xl px-5 py-5 sm:px-7 sm:py-6 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between mb-6"><div><p className="pi-kicker">Recruiter workspace</p><h1 className="text-3xl font-bold text-[#0F172A] mt-1">Interview history</h1><p className="text-slate-600 mt-2 max-w-xl">Review evidence, add notes, and record a human decision.</p></div><a href="/" className="pi-control inline-flex items-center justify-center rounded-lg bg-[#0E7490] px-4 text-sm font-semibold text-white hover:bg-[#155E75]">New interview</a></div>
...
function Metric({ label, value, tone = 'text-[#0E7490]' }: { label: string; value: string; tone?: string }) {
  return <div className="pi-surface rounded-xl p-4"><p className="text-xs uppercase tracking-wider text-slate-500">{label}</p><p className={`text-2xl font-bold mt-1 ${tone}`}>{value}</p></div>
}
