...
function CandidatePicker() {
  ...
  return (
    <div className="pi-surface rounded-2xl p-6 sm:p-8 w-full max-w-2xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-sm font-semibold text-[#1E3A5F] uppercase tracking-widest">Select a Candidate</h3>
        <span className="text-xs text-[#2563EB] font-medium bg-blue-50 px-3 py-1 rounded-full">Real mission data</span>
      </div>

      <div className="flex flex-col gap-3 mb-6">
        {candidates.length === 0 && !error && (
          <div className="text-center text-slate-500 text-sm py-6">Loading candidates from the live backend...</div>
        )}
        {candidates.map((c, i) => (
          <button
            key={c.member.id}
            onClick={() => setSelectedIdx(i)}
            className={`flex flex-wrap sm:flex-nowrap items-center gap-3 p-4 rounded-2xl border-2 text-left cursor-pointer
              transition-all duration-200 ${selectedIdx === i
          ...
          >
            ...
            <span className="shrink-0 text-xs font-medium text-[#2563EB] bg-blue-50 px-3 py-1 rounded-full">
      ...
      <div className="grid sm:grid-cols-3 gap-4 mb-6">
        <label className="text-xs font-semibold text-slate-600">Focus
          <select value={settings.focus} onChange={e => setSettings(s => ({ ...s, focus: e.target.value }))} className="mt-1 w-full rounded-xl border border-[#CBD5E1] bg-white px-4 py-2 text-sm">
            <option>Candidate project work</option>
            <option>System design</option>
            <option>Debugging and delivery</option>
          </select>
        </label>
        <label className="text-xs font-semibold text-slate-600">Length
          <select value={settings.duration} onChange={e => setSettings(s => ({ ...s, duration: e.target.value as InterviewSettings['duration'] }))} className="mt-1 w-full rounded-xl border border-[#CBD5E1] bg-white px-4 py-2 text-sm">
            <option value="short">Short</option>
            <option value="standard">Standard</option>
            <option value="deep">Deep dive</option>
          </select>
        </label>
        <label className="text-xs font-semibold text-slate-600">Style
          <select value={settings.style} onChange={e => setSettings(s => ({ ...s, style: e.target.value as InterviewSettings['style'] }))} className="mt-1 w-full rounded-xl border border-[#CBD5E1] bg-white px-4 py-2 text-sm">
      ...
      {error && (
        <div className="text-sm text-red-700 bg-red-50 border border-red-200 rounded-2xl p-4 mb-6 flex items-center justify-between gap-3">
          <span>{error}</span>
          <button type="button" onClick={loadCandidates} className="pi-control shrink-0 px-4 text-xs font-semibold border border-red-200 bg-white">Retry</button>
        </div>
      )}
      ...
      <button
        ...
        className="w-full py-3.5 bg-[#1E3A5F] text-white font-semibold rounded-2xl cursor-pointer
...
function HeroMock() {
  return (
    <div className="relative w-full max-w-lg mx-auto">
      <div className="relative rounded-3xl bg-white border border-[#BAE6FD] shadow-2xl shadow-cyan-950/10 overflow-hidden">
        <div className="flex items-center gap-3 px-5 py-4 border-b border-[#E4E7EB] bg-[#F8FAFC]">
          <div className="w-10 h-10 rounded-full bg-[#0E7490] flex items-center justify-center text-white font-bold text-sm shadow-sm">AI</div>
          <div>
            <div className="flex items-center gap-2">
              <span className="font-semibold text-sm text-[#0F172A]">Alex</span>
              <span className="text-[11px] font-medium bg-blue-50 text-[#2563EB] px-2 py-0.5 rounded-full">Technical Interviewer</span>
            </div>
        ...
        <div className="px-5 py-4 flex flex-col gap-3">
          <div className="flex justify-start">
            <div className="max-w-[85%] bg-white border border-[#E4E7EB] rounded-2xl rounded-bl-xl px-4 py-3 text-sm leading-relaxed text-[#0F172A]">
              Nice — embedding the intent classifier is a solid move. Building on that, how did you tune the retrieval threshold so recall didn't collapse on your Day 7 project? Walk me through your actual approach.
            </div>
          </div>
          <div className="flex justify-end">
            <div className="max-w-[85%] bg-[#1E3A5F] text-white rounded-2xl rounded-br-xl px-4 py-3 text-sm leading-relaxed">
              I iterated on eval sets per category — started strict for finance terms, then relaxed the top-k so the LLM could rerank ambiguous queries.
            </div>
          </div>
          <div className="flex justify-start">
            <div className="bg-white border border-[#E4E7EB] rounded-2xl rounded-bl-xl px-4 py-3 flex items-center gap-1.5">
      ...
      <div className="absolute -bottom-6 -left-4 sm:-left-8 bg-white border border-[#E4E7EB] rounded-2xl shadow-xl shadow-slate-900/5 px-4 py-3 hidden sm:block">
      ...
      <div className="absolute -top-4 -right-3 bg-white border border-[#E4E7EB] rounded-2xl shadow-xl shadow-slate-900/5 px-4 py-2 hidden sm:block">
...
/* ── Landing body ───────────────────────────────────────────────────── */
const FEATURES = [
  { icon: <IconChat />, title: 'Conducts real conversations', body: 'Follow-ups reference what the candidate just said — calling back earlier answers instead of re-running a static checklist.' },
  { icon: <IconLayers />, title: 'Structured by a real curriculum', body: 'Interview plans are built from the candidate\'s actual missions, tools, and objectives — not generic canned questions.' },
  { icon: <IconBolt />, title: 'Adaptive to experience', body: 'Persona tuning per seniority: encouraging for juniors, system-design depth for seniors, implementation focus for mid-level.' },
  { icon: <IconDoc />, title: 'Wraps up with real feedback', body: 'Ends with a structured report — summary, strengths, gaps, and next steps — so interviews are useful, not just a log of questions.' },
  { icon: <IconGauge />, title: 'Paced, not interrogating', body: 'Detects "I don\'t know" and pivots gracefully instead of hammering the same topic. Momentum matters in hiring.' },
  { icon: <IconShield />, title: 'Resilient for live demos', body: 'OpenRouter → local Ollama → offline mock fallback. No crashes if the network or a key fails mid-demo.' },
]
  ...
  return (
    <main className="min-h-dvh text-[#0F172A] overflow-x-clip">
      ...
      <header className="sticky top-0 z-20 bg-white/90 backdrop-blur-xl border-b border-[#E4E7EB]">
        <nav className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          <a href="#top" className="flex items-center gap-2.5 cursor-pointer">
            <div className="w-8 h-8 rounded-lg bg-[#1E3A5F] flex items-center justify-center text-white text-sm font-bold shadow-sm">PI</div>
          ...
          <a href="#start" className="pi-control px-5 bg-[#0E7490] text-white text-sm font-semibold rounded-lg cursor-pointer transition-all duration-200 shadow-sm hover:bg-[#155E75] active:scale-[0.98]">
            Start interview
          </a>
        </nav>
      </header>
      ...
      <section id="top" className="max-w-6xl mx-auto px-4 sm:px-6 pt-14 sm:pt-20 pb-16 grid lg:grid-cols-2 gap-12 items-center min-h-[calc(100svh-4rem)]">
        <div>
          <span className="landing-reveal inline-flex items-center gap-2 text-xs font-semibold text-[#0E7490] bg-cyan-50 border border-cyan-100 rounded-full px-3.5 py-1.5">
          ...
          <p className="landing-reveal landing-reveal-2 mt-5 text-lg text-slate-600 leading-relaxed max-w-xl">
            ProbeIQ conducts realistic multi-turn technical interviews, grounded in what a candidate actually built — then delivers structured, honest feedback your team can act on.
          </p>
          <div className="landing-reveal landing-reveal-3 mt-8 flex flex-wrap gap-3">
            <a href="#start" className="pi-control px-7 inline-flex items-center bg-[#0E7490] text-white font-semibold rounded-lg cursor-pointer transition-all duration-200 shadow-lg shadow-cyan-950/10 hover:bg-[#155E75] hover:-translate-y-0.5 active:scale-[0.98]">
              Start an interview <IconArrow className="inline" />
            </a>
            <a href="#how" className="pi-control px-7 inline-flex items-center bg-white text-[#0F172A] font-semibold rounded-lg cursor-pointer border border-[#BAE6FD] transition-all duration-200 hover:border-[#0891B2] hover:text-[#0E7490] active:scale-[0.98]">
              See how it works
            </a>
          </div>
          ...
          <div className="mt-12 grid md:grid-cols-3 gap-6">
            {STEPS.map(s => (
              <div key={s.n} className="bg-[#F8FAFC] border border-[#E4E7EB] rounded-2xl p-6">
                <div className="text-3xl font-extrabold text-[#1E3A5F]">{s.n}</div>
      ...
      <section id="features" className="scroll-mt-24 max-w-6xl mx-auto px-4 sm:px-6 py-20">
        ...
        <div className="mt-12 grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {FEATURES.map(f => (
            <div key={f.title} className="bg-white rounded-2xl border border-[#E4E7EB] p-6 shadow-sm hover:shadow-md hover:border-[#2563EB]/40 hover:-translate-y-1 transition-all duration-200">
              <div className="w-11 h-11 rounded-xl bg-[#1E3A5F] text-white flex items-center justify-center shadow-sm">{f.icon}</div>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-20 grid lg:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <p className="mt-4 text-slate-600 leading-relaxed">
              When the last question is asked, ProbeIQ assembles a structured assessment: a candid summary, quantified strengths, areas to grow, and concrete next steps.
            </p>
      ...
      <section id="start" className="scroll-mt-24 max-w-6xl mx-auto px-4 sm:px-6 py-24 text-center">
        <div className="relative rounded-3xl overflow-hidden bg-[#1E3A5F] px-6 py-16 sm:py-20 shadow-xl shadow-slate-900/20">
          ...
          <div className="relative">
            <h2 className="text-3xl sm:text-4xl font-bold text-white tracking-tight">Try a live interview — on your own candidate data.</h2>
      ...
      <footer className="border-t border-[#E4E7EB] bg-white">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-lg bg-[#1E3A5F] flex items-center justify-center text-white text-[11px] font-bold">PI</div>
